...
export default function MFASettingsPage() {
  ...
  const fetchMfaStatus = async () => {
    setLoading(true);
    const result = await apiGet<MfaStatus>('/api/account/mfa/status', 'mfa');
  ...
  const handleStartSetup = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || t(language, `${prefix}.errors.setupFailed`));
      }
    } catch (err) {
      console.error('Failed to start MFA setup:', err);
      setError(t(language, `${prefix}.errors.setupFailed`));
    } finally {
  ...
  const handleEnableMfa = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || t(language, `${prefix}.errors.invalidCode`));
      }
    } catch (err) {
      console.error('Failed to enable MFA:', err);
      setError(t(language, `${prefix}.errors.enableFailed`));
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDisableMfa = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(errorData.error || t(language, `${prefix}.errors.disableFailed`));
      }
    } catch (err) {
      console.error('Failed to disable MFA:', err);
      setError(t(language, `${prefix}.errors.disableFailed`));
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleRegenerateCodes = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(errorData.error || t(language, `${prefix}.errors.regenerateFailed`));
      }
    } catch (err) {
      console.error('Failed to regenerate codes:', err);
      setError(t(language, `${prefix}.errors.regenerateFailed`));
    } finally {
      setIsSubmitting(false);
    }
  };
